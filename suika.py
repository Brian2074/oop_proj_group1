...
# %%
def set_bbox_inches_tight(fig, ratio_margin=0.01):#調整邊框
    assert 0 <= ratio_margin <= 1   #比例邊距
...
def set_ball_mrI(ball):#計算質量半徑轉動慣量
    ball_type = ball['type']
    ball['m'] = 1
    ball['1/m'] = 1 /ball['m']
...
gamma_v = .9 ** (dt * inv_n_substep)#線、角速度衰減係數
gamma_omega = .2 ** (dt * inv_n_substep)

elasticity_collision = 1 # max 2 --> complete elastic彈性、摩擦係數
...
@njit(fastmath=True, cache=True)
def get_contact_(ball_, n_ball, contact_):#距離小於半徑和為接觸,同類須合併
